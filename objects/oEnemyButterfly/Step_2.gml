/// @description Handles all movement, attack, and transformation logic for oEnemyButterfly
/// This Step event manages the butterfly's behavior, including movement, attack patterns, transformation, and interaction with global and controller flags.
/// It checks game state, controller variables, and global flags to determine how the butterfly should act each frame.

// Inherit parent event logic (from oEnemy)
event_inherited();

// Destroy rogue butterflies if they leave the screen
if rogue = 1 and (y > 592 or x < -16 or x > 464){
	instance_destroy();
}

// Limit the number of enemy shots and spawn them at specific alarm intervals
...
// Calculate breathing animation offsets for butterfly movement
breathex = xstart + ((global.breathe/120)*(48*((xstart - 224)/144))) + Controller.x - 16;
breathey = ystart + ((global.breathe/120)*(48*((ystart - 64)/144)));

// Adjust speed for fast mode and secondary dive
if global.fast = 1 and dive2 > 0{
	spd = 6;
} else {
	spd = 3;
}

// Transformation logic: check if butterfly should transform based on controller and global flags
if global.transnum > 0{
	if dive = 0 and irandom(5) = 0 and global.divecap > 0 and uprohib = 0 and global.prohib = 0 and global.transform = 0 and Ship.shipStatus = ShipState.ACTIVE and Ship.regain = 0
...
// Convoy movement logic: butterflies move in formation until they dive
if dive = 0 and enter = 0{
	depth = 100;
	...
	x = breathex;
	y = breathey;
	// Check if butterfly should start a dive attack
	if global.divecap > 0 and global.open = 0 and Ship.alarm[4] = -1{
		if irandom(10) = 0 and global.prohib = 0 and uprohib = 0 and escort = 0 and alarm[2] = -1 and Ship.shipStatus == ShipState.ACTIVE and Ship.regain = 0{
			dive = 1; direction = 90; global.prohib = 1; Controller.alarm[0] = 15; alarm[1] = 90; sound_stop(GDive); sound_play(GDive);
			if xstart > 224{path_start(Butterfly1,spd,0,false)} else{path_start(Butterfly1Flip,spd,0,false)}
		}
	}
}

// Charger logic: handles butterfly dive attacks and looping movement
if trans = 0{
	// Charger without escort
	if dive = 1 and escort = 0 and enter = 0{
		depth = -102;
		if loop = 0{
			// End path and start looping if butterfly reaches certain position
		...
		if loop = 1{
			...
			// Reset if butterfly leaves screen
		...
		if loop = 5{
			// Return to formation after attack or if player is dead
			if instance_number(Bee) + instance_number(Butterfly) + instance_number(Boss) > global.lastattack or y > -16  or (Ship.shipStatus == ShipState.DEAD or Ship.regain = 1){
				...
				if global.fast = 1 and dive2 = 1{
					if xstart > 224{path_start(Butterfly2,spd,0,false)} else{path_start(Butterfly2Flip,spd,0,false)}
				}
				else{
					if global.fast = 1 and dive2 = 2{
						...
						if xstart > 224{path_start(Butterfly1Alt,spd,0,false)} else{path_start(Butterfly1AltFlip,spd,0,false)}
					}
				}
			}
		}
	}
	// Charger with escort logic
	if dive = 1 and escort = 1 and enter = 0{
		depth = -101;
		if loop = 0{
		...
		if y < ystart + 160 and loop = 0{}
			...
			if loop = 0{
				if y < 480{path_end(); if direction < 270{direction = direction + 1}; if direction > 270{direction = direction - 1}; speed = spd}
					...
					if xstart > 224{path_start(BeeLoop,spd,0,false); loop = 2} else{path_start(BeeLoopFlip,spd,0,false); loop = 2}
				}
			}
			if loop = 1{
				path_end();
			...
			if loop = 2{
				...
				if y > 592+32+32{
					...
					if xstart > 224{path_start(Bee1Alt,spd,0,false)} else{path_start(Bee1AltFlip,spd,0,false)}
				}
			}
		}
	}
...
// Fast entry logic for butterflies
if global.fastenter = 1 and global.open = 1{
	if fasty > 0{fasty = fasty - 1}else{path_speed = 12; speed = 12}
}

// Timer logic for entry and movement
if timey > 0{timey = timey - 1};

// Entry logic: move butterfly to its starting position
if enter = 1{
	if global.pattern = 0 or 1{
		if y < (272-16)*global.scale{
			if rogue = 0{
				if ((global.wave = 0 and direction < 180) or (global.wave > 0 and direction = 90)) and (global.pattern = 0 or global.pattern = 2 or (global.wave = 0 or timey = 0)) and goto = 0{
					path_end(); 
					// Assign starting positions based on numb value
		...
		// Move butterfly to its target position
		if goto = 1{
...
// Execute rogue turn script for rogue butterflies
